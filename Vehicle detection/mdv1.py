...
def CenterCalculation(frame, contour):
    center = [0, 0]
    if USERECT:
...
if __name__ == '__main__':
    if (len(sys.argv) < 2):
        print("Please specific the path of the video and the road environment with none vehicles.")
        sys.exit()
    ...
    firstFrame = cv2.resize(frame, (960, 540), interpolation=cv2.INTER_AREA)
    ...
    while True:
        ...
        if SHOWDIVIDELINE:
            cv2.line(frame,(0,frameCenter[1]),(1920,frameCenter[1]),(255,0,0),1)
            cv2.line(frame,(frameCenter[0], 0),(frameCenter[0], 1280),(255,0,0),1)
        ...
        for c in contours:
            ...
            # center of contour
            center = CenterCalculation(frame, c)
